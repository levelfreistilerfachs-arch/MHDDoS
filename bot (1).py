...
def notify_groups_bot_started():
    ...
    for group_id in groups:
        try:
            bot.send_message(
                group_id,
                "✅ *¡El bot ha sido revivido locas!*\n\n"
                "Ya puedes seguir dañando salas de Tik Tok y subiendole el ping a tus primitos.\n\n"
                "¡Gracias por su paciencia hijos de la Verga! 💪",
                parse_mode="Markdown",
            )
        except Exception as e:
            print(f"No se pudo enviar mensaje al grupo {group_id}: {str(e)}")
    ...
    try:
        ...
        bot.reply_to(
            ...
            (
                ...
                "*Este bot fue creado por Botsito de los Botsitos* 🎮"
            ),
            reply_markup=markup,
            parse_mode="Markdown",
        )
...
def handle_stop_attack(call):
    ...
    if telegram_id in active_attacks:
        ...
        try:
            ...
            markup = InlineKeyboardMarkup()
            markup.add(InlineKeyboardButton("🔄 Subir Ping 999 de nuevo", callback_data=f"restart_attack_{telegram_id}"))

            bot.edit_message_text(
                "*[⛔] *PING 999 PARADO* [⛔]*\n\n"
                "¿Quieres subir el ping nuevamente? Tienes **20 segundos** para decidir.",
...
def handle_restart_attack(call):
    ...
    if telegram_id in cooldowns and time.time() - cooldowns[telegram_id] < 20:
        ...
        try:
            ...
            if duration > 999:
                bot.answer_callback_query(call.id, "❌ *La duración máxima permitida es de 999 segundos (16,36 minutos).*")
                return
...
def handle_help(message):
    ...
    bot.send_message(
        ...
        (
            ...
            "2. `/botsito <TIPO> <IP/HOST:PUERTO> <HILOS> <MS>`: Pone ping 999 a la partida.\n"
...
def handle_timeactive(message):
    ...
    bot.reply_to(
        ...
        (
            ...
            "🚀 *Recuerda que el bot se desactiva automáticamente después de 140 minutos.*"
        ),
        parse_mode="Markdown"
    )
